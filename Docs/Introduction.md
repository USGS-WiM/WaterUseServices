...
The Water use Services API performs multiple high level procedures which include; database and geospatial queries and service requests in order to compile and create simple objects that can be consumed by custom client applications. As documented by this API, which can also serve as an example URL builder, the water use service documentation API follows [RESTful](http://en.wikipedia.org/wiki/Representational_state_transfer) principles to ensure scalability and predictable URLs. JSON is returned as its primary API response, including errors and example results and summaries for each resource. This API is intended to provide some guidance in working with the services, however some methods may only provide visual examples due to authentication requirements or limitations of the API. 
